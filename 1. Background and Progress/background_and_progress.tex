...
\section{Iteration 4: metaSNE \& Principle Component Analysis}
	...
	The test set, above, was a number of uniformly generated $x$ and $y$ points that when plotted produced a dataset that consisted of two identical triangles that had been rotated by differing amounts, and three idential squares that had likewise been rotated by differing degrees with differing offsets.
	\par 
	...
	\par 
	With the scatterplots, in order to do this, the (x, y) coordinates needed to first be transformed into an image with discrete dimensions. For example, a 500 pixel by 500 pixel image. 
	\par 
...
\\\
	...
	\par
	The second method attempted was to first apply a Principle Component Analysis, PCA, transformation to the scatterplot in an attempt to remove the majority of the rotational differences. 
	\par 
	PCA is a statistical procedure that converts the set of points along a number of principle compoents. The largest principle component has the largest possible variablity in the data, and each succeeding component has the highest variance possible under the constraint of those preceeding it.
	\par 
	...
	\par 
	This second method was far more successful than the first in accurately distinguishing the differences and similarities in the original dataset once plotted, however upon application of tSNE the points were indistunguishably plotted.
	\\\
		\begin{figure}[H]
    			\centering	
